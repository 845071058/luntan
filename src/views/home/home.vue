<template>
  <div class="home">
    <div class="circleDetail" v-if="isMyCircle">
      <div class="head">
        <div class="head-img">
    ...
    <div class="recomCircle" v-if="!isMyCircle">
      <div class="recomCircleItem" v-for="(item, index) in recomCircle" :key="index">
        <div class="head">
          <div class="headImg"><img :src="item.circleImg" alt="" /></div>
          <div class="headBtn">
            <van-button type="default" class="visit-btn" @click="onCircle(item)">加入</van-button>
          </div>
        </div>
        <div class="text">
          <h3>{{ item.finalName }}</h3>
          <div class="total">
            <p>成员：{{ item.totalUser }}</p>
            <p>发帖：{{ item.totalBlog }}</p>
          </div>
          <span>{{ item.circleDesc }}</span>
        </div>
        <div class="blog" v-if="item.blogs && item.blogs.length > 0">
          <div class="blogItem" v-for="(obj, keys) in item.blogs" :key="keys">
    ...
    <van-dialog v-model="vanDialog" title="修改圈子信息" show-cancel-button @confirm="updateCircle">
    ...
    <van-dialog v-model="vanDialogs" title="圈友通讯录" show-cancel-button @confirm="updateCircleUserInfo">
      <van-field label="标签" label-width="40px" placeholder="职业，产品，或爱好" v-model="upCircleUserInfo.label" />
      <van-field label="电话" label-width="40px" placeholder="以便他人更好的联系您哦" maxlength="11" type="tel" v-model="upCircleUserInfo.mobile" />
    </van-dialog>
    ...
    <!-- 查看回复详情 -->
    <van-overlay :show="replyShowDetail" z-index="1000" :lock-scroll="false">
      <div class="commentBox" @click.stop>
        <div class="commentTit">
  ...
  export default {
    ...
    methods: {
        ...
        this.Api.myDadPage({
          ...
          if (res.success) {
            if (res.data) {
              ...
              this.getCircleDetail(1);
            } else {
              this.Api.recomCircle({
                userId: this.userId
              }).then(res => {
                if (res.success) {
                  this.recomCircle = res.data;
                }
              });
            }
          } else {
      ...
      getCircleDetail(type) {
        this.Api.getCircleDetail({
          ...
          if (res.success) {
            ...
            this.$route.meta.title = res.data.circleName;
            this.isMaster = res.data.isMaster;
            if (res.data.isOpen === 1) this.vanDialogs = true
            if (type === 2) return
      ...
      goUserDetail(userInfo) {
        let userId = userInfo.userId ? userInfo.userId : userInfo.creater ? userInfo.creater : userInfo.createrId;
        this.$router.push({
          path: 'circleUserInfo',
          query: {
            toUserId: userId,
      ...
      updateCircleUserInfo() {
        this.upCircleUserInfo.circleId = this.circleId
        this.upCircleUserInfo.userId = this.userId
        this.Api.updMyCircleInfo(this.upCircleUserInfo).then(res => {
          if (res.success) {
            this.$toast('修改成功');
          } else {
            this.$toast(res.msg);
          }
          this.vanDialogs = false
        });
      }
    }
  };
